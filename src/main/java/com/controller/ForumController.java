    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(forum !=null){
            ...
            BeanUtils.copyProperties( forum , view );//把实体数据重构到view中
                ...
                //级联表
                HuodongchengbanfangEntity huodongchengbanfang = huodongchengbanfangService.selectById(forum.getHuodongchengbanfangId());
                if(huodongchengbanfang != null){
                    BeanUtils.copyProperties( huodongchengbanfang , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setHuodongchengbanfangId(huodongchengbanfang.getId());
                }
                //级联表
                TuanweiEntity tuanwei = tuanweiService.selectById(forum.getTuanweiId());
                if(tuanwei != null){
                    BeanUtils.copyProperties( tuanwei , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setTuanweiId(tuanwei.getId());
                }
                //级联表
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setHuodongchengbanfangId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        else if("团委".equals(role))
            forum.setTuanweiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("huodongchengbanfang_id", forum.getHuodongchengbanfangId())
            .eq("tuanwei_id", forum.getTuanweiId())
...
//                            forumEntity.setHuodongchengbanfangId(Integer.valueOf(data.get(0)));   //活动承办方 要改的
//                            forumEntity.setTuanweiId(Integer.valueOf(data.get(0)));   //团委 要改的
        ...
        ForumEntity forum = forumService.selectById(id);
            if(forum !=null){
                ...
                //级联表
                    HuodongchengbanfangEntity huodongchengbanfang = huodongchengbanfangService.selectById(forum.getHuodongchengbanfangId());
                if(huodongchengbanfang != null){
                    BeanUtils.copyProperties( huodongchengbanfang , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setHuodongchengbanfangId(huodongchengbanfang.getId());
                }
                //级联表
                    TuanweiEntity tuanwei = tuanweiService.selectById(forum.getTuanweiId());
                if(tuanwei != null){
                    BeanUtils.copyProperties( tuanwei , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setTuanweiId(tuanwei.getId());
                }
                //级联表
                    ZhiyuanzheEntity zhiyuanzhe = zhiyuanzheService.selectById(forum.getZhiyuanzheId());
